...
public class ApplicationControllerTests
{
    ...
    public async Task SubmitApplication_WhenSuccess_ReturnsOk()
    {
        ...
        var dto = new ApplicationDto { SchoolName = "Test University" };
        var fileMock = new Mock<IFormFile>();
        var expectedId = Guid.NewGuid();

   
        _appServiceMock.Setup(x => x.SubmitAsync(userId, dto, fileMock.Object))
            .ReturnsAsync(ServiceResult<Guid>.Success(expectedId));
        
        var result = await _controller.SubmitApplication(dto, fileMock.Object);
